...
Vector<const char *> VulkanSDLWindowSystem::getInstanceExtensions()
{
    uint32_t extensionCount = 0;
    const char *const *extensionNames = SDL_Vulkan_GetInstanceExtensions(&extensionCount);
    assert(extensionNames && extensionCount > 0);

    return Vector<const char *>(extensionNames, extensionNames + extensionCount);
}
...
vec2 VulkanSDLWindowSystem::getWindowSize()
{
    int width, height;
