...
void PipelineBuilder::setBindingDescription(uint32_t binding, uint32_t stride, VkVertexInputRate inputRate) { bindingDescriptions.push_back({binding, stride, inputRate}); }

void PipelineBuilder::setAttributeDescription(uint32_t location, uint32_t binding, VkFormat format, uint32_t offset) { attributeDescriptions.push_back({location, binding, format, offset}); }

void PipelineBuilder::setPipelineLayout(VkPipelineLayout &layout) { pipelineLayout = layout; }

void PipelineBuilder::clearShaders() { shaderStages.clear(); }
...
void PipelineBuilder::setTopology(VkPrimitiveTopology topology) { inputAssemblyState.topology = topology; }

void PipelineBuilder::setPatchControlPoints(uint32_t points) { tessellationState.patchControlPoints = points; }

void PipelineBuilder::setMultisampleCount(VkSampleCountFlagBits samples) { multisampleState.rasterizationSamples = samples; }

VkPipeline PipelineBuilder::build(VkDevice device, uint32_t colorAttachmentCount)
{
