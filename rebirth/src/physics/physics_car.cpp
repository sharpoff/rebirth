...
void PhysicsCar::initialize(rebirth::PhysicsSystem &physicsSystem, vec3 position)
{
    JPH::PhysicsSystem &system = physicsSystem.getPhysicsSystem();
    ...
    // create body
    // clang-format off
    JPH::Ref<JPH::Shape> car_shape = JPH::OffsetCenterOfMassShapeSettings(JPH::Vec3(0, -half_vehicle_height, 0),
        new JPH::BoxShape(JPH::Vec3(half_vehicle_width, half_vehicle_height, half_vehicle_length))).Create().Get();
    // clang-format on

    bodyInterface = &system.GetBodyInterface();

    JPH::BodyCreationSettings car_body_settings(
        car_shape, toJolt(position), JPH::Quat::sRotation(JPH::Vec3::sAxisZ(), sInitialRollAngle),
    ...
    bodyInterface->AddBody(mCarBody->GetID(), JPH::EActivation::Activate);

    rigidBodyId = physicsSystem.addShape(car_shape, mCarBody->GetID());
    ...
    JPH::Vec3 front_suspension_dir =
        JPH::Vec3(
            JPH::Tan(sFrontSuspensionSidewaysAngle), -1, JPH::Tan(sFrontSuspensionForwardAngle)
        )
            .Normalized();
    JPH::Vec3 front_steering_axis =
    ...
    mVehicleConstraint = new JPH::VehicleConstraint(*mCarBody, vehicle);
    ...
    static_cast<JPH::WheeledVehicleController *>(mVehicleConstraint->GetController())
        ->SetTireMaxImpulseCallback([](uint, float &outLongitudinalImpulse,
                                       float &outLateralImpulse, float inSuspensionImpulse,
                                       float inLongitudinalFriction, float inLateralFriction, float,
                                       float, float) {
    ...
    system.AddConstraint(mVehicleConstraint);
    system.AddStepListener(mVehicleConstraint);

    // UpdateCameraPivot();
}

void PhysicsCar::update(float deltaTime)
{
...
void PhysicsCar::processInput()
{
    ...
    if (mPreviousForward * mForward < 0.0f) {
        ...
        float velocity =
            (mCarBody->GetRotation().Conjugated() * mCarBody->GetLinearVelocity()).GetZ();
...
Transform PhysicsCar::getWheelTransform(int wheel)
{
    JPH::RMat44 wheel_transform =
        mVehicleConstraint->GetWheelWorldTransform(wheel, JPH::Vec3::sAxisY(), JPH::Vec3::sAxisX());

	Transform transform = Transform(toGLM(wheel_transform));
	transform.scale(vec3(getWheelWidth(wheel)));
    return transform;
}

float PhysicsCar::getWheelRadius(int wheel)
{
    const JPH::WheelSettings *settings = mVehicleConstraint->GetWheels()[wheel]->GetSettings();
    return settings->mWidth;
}

float PhysicsCar::getWheelWidth(int wheel)
{
    const JPH::WheelSettings *settings = mVehicleConstraint->GetWheels()[wheel]->GetSettings();
    return settings->mRadius;
}

const vec3 PhysicsCar::getPosition() const
{
	return toGLM(mCarBody->GetPosition());
}

const quat PhysicsCar::getRotation() const
{
	return toGLM(mCarBody->GetRotation());
}

void PhysicsCar::setPosition(vec3 position)
{
    bodyInterface->SetPosition(mCarBody->GetID(), toJolt(position), JPH::EActivation::Activate);
}

void PhysicsCar::setRotation(quat rotation)
{
    bodyInterface->SetRotation(mCarBody->GetID(), toJolt(rotation), JPH::EActivation::Activate);
}
